...
## 5. 追加課題（時間が余ったら）（目安時間：10分）

いずれも **`lib/counter_page.dart` を丸ごと差し替え**、`main.dart` はハンズオンのままでよい（`home: CounterPage()`）。パッケージ名は自分の `pubspec.yaml` の `name:` に合わせる。

### Easy — デクリメントボタンを追加

**課題:** 加算 FAB に加え、減算用の FAB を置き、0 未満にならないようにする（任意で `0` でクランプ）。

**解答（`lib/counter_page.dart` 全文）:**
...
**補足:** `FloatingActionButton` が複数あるときは **`heroTag`** を別名にしないと Hero アニメーションで例外になる。

---

### Medium — `build` 内で副作用を書かず、表示用の getter に寄せる

**課題:** 表示文字列は `_label` のような getter にまとめ、`build` では `Text(_label)` だけにする。
...
**補足:** ハンズオンの `test/counter_page_test.dart` は **`'0'` / `'1'`** を探しているため、この解答では **`find.text('Count: 0')`** にテストを直すか、表示を `'$_count'` のままにして getter は別用途に使うか、どちらかに合わせる。

---

### Hard — `ValueNotifier<int>` + `ValueListenableBuilder`（`dispose` で `dispose`）

**課題:** カウンターの整数は `ValueNotifier` が保持し、表示は `ValueListenableBuilder`。増加は `setState` ではなく `_count.value++`。`dispose` で `_count.dispose()` を必ず呼ぶ。
...
**補足:** この形では `setState` を使わないため、**Widget テストで `tap` のあと `pump()` すれば**従来どおり期待値が合う。
